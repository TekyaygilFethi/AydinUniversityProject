...
namespace AydinUniversityProject.Admin.Views.FavouriteFeedsCollectionView{
    public partial class FavouriteFeedsCollectionView : XtraUserControl {
        public FavouriteFeedsCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<AydinUniversityProject.Admin.ViewModels.FavouriteFeedsCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as AydinUniversityProject.Data.POCOs.FavouriteFeeds,
