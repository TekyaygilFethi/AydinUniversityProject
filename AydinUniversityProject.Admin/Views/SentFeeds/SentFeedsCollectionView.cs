...
namespace AydinUniversityProject.Admin.Views.SentFeedsCollectionView{
    public partial class SentFeedsCollectionView : XtraUserControl {
        public SentFeedsCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<AydinUniversityProject.Admin.ViewModels.SentFeedsCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as AydinUniversityProject.Data.POCOs.SentFeeds,
